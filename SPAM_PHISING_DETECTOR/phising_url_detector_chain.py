...
class URLSchema(BaseModel):
    ...
    explaination : str = Field(description="This field gives the explaination as in why the url is phising in 1-2 lines")
...
{{
  ...
  "explaination": "<one- or two-line explanation why the URL is flagged or not>"
}}

Use the exact field names: `is_phising` (boolean) and `explaination` (string). Keep `explaination` concise (1–2 lines).
...
- If uncertain, lean towards true and explain briefly.

Output constraints:  
- Only return JSON.  
- Explanation must mention key reasons briefly.

Examples:  
Input: https://www.paypal.com/signin  
Output: {{"is_phising": false, "explaination": "Domain is the official paypal.com host and path is normal; no obfuscation detected."}}

Input: http://paypal-secure-login.com/account/verify  
Output: {{"is_phising": true, "explaination": "Suspicious domain uses brand name with hyphen and path contains 'verify' — likely phishing."}}
